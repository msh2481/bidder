...
async def cmd_principles(message: Message):
    ...
    if not raw:
        await message.answer(
            "You haven't set up any principles yet. Use /update_principles to add them."
        )
        return
    ...
    status_parts = [f"📚 You have {count} principle{'s' if count != 1 else ''} saved."]

    if time_config and "time" in time_config:
        reminder_time = time_config["time"]
        status_parts.append(f"⏰ Daily reminder set for {reminder_time} (server time)")
    else:
        status_parts.append("⏰ No daily reminder set. Use /reminder to set one.")
